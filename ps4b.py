...
class Message(object):
    ...
    def apply_pad(self, pad):
        ...
        '''
        if len(pad) != len(self.message_text):
            raise ValueError("Pad length must match message length")
        
        result = ""
        for char, shift in zip(self.message_text, pad):
...
class PlaintextMessage(Message):
    def __init__(self, input_text, pad=None):
        ...
        if pad is None:
            ...
            if len(pad) != len(input_text):
                raise ValueError("Pad length must match message length")
            self.pad = pad.copy()  # Make a copy to prevent mutation
    ...
    def change_pad(self, new_pad):
        ...
        Returns: nothing
        '''
        if len(new_pad) != len(self.message_text):
            raise ValueError("New pad length must match message length")
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        ...
        if len(pad) != len(self.message_text):
            raise ValueError("Pad length must match message length")
        # To decrypt, we use the negative of each shift value
